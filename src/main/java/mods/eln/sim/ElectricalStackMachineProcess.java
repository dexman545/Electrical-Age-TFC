...
public class ElectricalStackMachineProcess implements IProcess {
    ...
    public void smeltItem() {
        if (this.smeltCan()) {
        	...
        	if (recipe.input.getItem() instanceof IFood)
        	{
        		ItemStack inputitem = inventory.getStackInSlot(inputSlotId);
        		if(inputitem.hasTagCompound() && inputitem.getTagCompound().hasKey("foodWeight") 
        				&& inputitem.getTagCompound().hasKey("foodDecay"))
        		{
        			ItemStack[] outputcopy = recipe.getOutputCopy();
	    			float flourWeight = inputitem.getTagCompound().getFloat("foodWeight");
	    			float flourDecay = inputitem.getTagCompound().getFloat("foodDecay");
	    			ItemStack out = ItemFoodTFC.createTag(outputcopy[0], flourWeight, flourDecay);
        			ItemStack outputs[] = { out };
    	            Utils.tryPutStackInInventory(outputs, inventory,outSlotIdList);
