...
public class EggIncubatorElement extends TransparentNodeElement {
	...
	class EggIncubatorProcess implements IProcess, INBTTReady {
		...
		void resetEnergy() {
			energy = 200000 + Math.random() * 50000;
		}
		
		public EntityAIBase SearchForAIBase(EntityChickenTFC chicken, int priority)
		{			
	        Iterator iterator = chicken.targetTasks.taskEntries.iterator();

	        while (iterator.hasNext())
	        {
	            EntityAITasks.EntityAITaskEntry entityaitaskentry = (EntityAITasks.EntityAITaskEntry)iterator.next();
	            if (entityaitaskentry.priority == priority)
	            {
		...
		public void process(double time) {
			...
			if (inventory.getStackInSlot(EggIncubatorContainer.EggSlotId) != null) {
				...
				if (energy <= 0) {
					...
					EntityChickenTFC chicken = new EntityChickenTFC(node.coordonate.world());
			    	try
					{
						Field field = EntityChickenTFC.class.getDeclaredField("sex");
						field.setAccessible(true);
						
						if (Math.random() <= 0.05f)
						{
							if (field.getInt(chicken) != 1)
							{							
								EntityAIBase aibase = SearchForAIBase(chicken, 2);
								if (aibase != null)
									chicken.targetTasks.removeTask(aibase);
								
								field.setInt(chicken, 1);
							}
						}
						else
						{
							if (field.getInt(chicken) != 0)
							{
								field.setInt(chicken, 0);
								chicken.targetTasks.addTask(2, new EntityAINearestAttackableTarget(chicken, EntityPlayer.class, 0, true));
							}
						}
